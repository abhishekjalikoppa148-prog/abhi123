...
export class EmailService {
  ...
  static sendPasswordResetEmail(email: string, name: string, resetUrl: string): Promise<boolean> {
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <h1 style="color: #e11d48;">Reset Your Password</h1>
        <p>Hi ${name},</p>
        <p>You recently requested to reset your password for your CelebrationCraft account. Click the button below to reset it.</p>
        <a href="${resetUrl}" style="display: inline-block; padding: 12px 24px; background: linear-gradient(to right, #e11d48, #9333ea); color: white; text-decoration: none; border-radius: 8px;">Reset Password</a>
    ...
    return this.sendEmail({
      to: email,
      subject: 'Reset Your Password - CelebrationCraft',
      html,
      text: `Reset your password at CelebrationCraft by going to: ${resetUrl}`
    });
  }
