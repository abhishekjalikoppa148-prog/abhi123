...
import { getUserByEmail, createPasswordResetToken } from '@/lib/mysql';
import { generateResetToken, getResetTokenExpiry, validateEmail, checkRateLimit } from '@/lib/auth';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (user) {
      ...
      await EmailService.sendPasswordResetEmail(user.email, user.name, resetUrl).catch(() => {});
    }
